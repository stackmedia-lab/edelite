    ...
    <div style={{ background: '#0A0A0F' }}>
      <section className="relative pt-32 pb-20 px-6 overflow-hidden" style={{ background: 'linear-gradient(135deg, #0A0A0F 0%, #0D1529 100%)' }}>
        ...
        <div className="relative z-10 max-w-4xl mx-auto text-center">
          <div className="text-xs uppercase tracking-widest mb-4" style={{ color: '#C9A84C' }}>Insights & Articles</div>
          <h1 className="text-5xl font-bold mb-6" style={{ fontFamily: 'Playfair Display, serif' }}>Our Blog</h1>
            ...
            {blogs.map((b) => (
              <Link key={b.slug} href={`/blog/${b.slug}`} className="group">
                <article
                  ...
                  style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(201,168,76,0.1)', transition: 'all 0.3s' }}
                >
                  ...
                  <div className="p-6 flex flex-col h-full">
                    <div
                      className="inline-flex px-3 py-1 rounded-full text-xs font-medium mb-3 self-start"
                      style={{ background: 'rgba(201,168,76,0.12)', color: '#C9A84C' }}
                    >
                      {b.tag}
                    </div>
                    <h2 className="font-semibold text-base leading-snug mb-3 line-clamp-2">{b.title}</h2>
                    <p className="text-sm leading-relaxed mb-4 line-clamp-3" style={{ color: 'rgba(255,255,255,0.5)' }}>{b.excerpt}</p>
                    <div className="flex items-center justify-between mt-auto">
                      <span className="text-xs" style={{ color: 'rgba(255,255,255,0.4)' }}>{b.author} · {b.time}</span>
                      <span className="text-xs font-medium" style={{ color: '#C9A84C' }}>Read →</span>
                    </div>
                  </div>
                </article>
              </Link>
